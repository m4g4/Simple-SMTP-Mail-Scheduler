 ...
 */
function echo_message_styles(): void {
    ?>
    <style>
        .smtp-mail-message {
 ...
 */
function decrypt_password( string $ciphertext ): string {
 ...
 */
function get_active_profile(): ?array {
    $active_profile_id = get_option( Constants::PROFILE_ACTIVE, null );
...
function get_profile_by_mail($from_email) {
    $profiles = get_option( Constants::PROFILES, [] );
...
function calculate_eta(int $queued, float $rate, string $unit): int {
    ...
    $slots = $queued / $rate;
    $duration_seconds = $slots * slot_time_seconds($unit);
    return time() + $duration_seconds;
}

function slot_time_seconds(string $unit): float {
    return match ($unit) {
        'day'    => 86400,
        'hour'   => 3600,
        'minute' => 60,
